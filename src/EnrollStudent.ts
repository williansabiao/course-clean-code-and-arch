...
export default class EnrollStudent {
  ...
  isStudentDuplicated(newStudent: any): boolean {
  ...
  execute(enrollmentRequest: enrollmentRequestType): any[] {
    ...
    if (!enrollmentModule) throw new Error('Module not found')

    const clazz = college.classes.find(
      (clazz) =>
    ...
    if (student.age.value <= enrollmentModule.minimumAge) throw new Error('Student below minimum age')

    if (
      this.enrollments.filter(
        (student) =>
          student.level === enrollmentRequest.level &&
          student.module === enrollmentRequest.module &&
          student.class === enrollmentRequest.class
      ).length >= clazz.capacity
    )
    ...
    const enrollment = {
      student,
      code: this.generateEnrollmentCode(enrollmentRequest.level, enrollmentRequest.module, enrollmentRequest.class),
